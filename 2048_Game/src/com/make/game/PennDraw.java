...
public final class PennDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	/**
	 * Set the window size to w-by-h pixels. This method resets the x- and y-scales,
	 * fonts, colors, pen radius, etc., and clears the screen
	 *
	...
	/**
	 * List all available fonts Code from
	...
	/**
	 * Set the font to the specified height in pixels. The conversion is approximate
	...
	public static void point(double x, double y) {
		...
		if (scaledPenRadius <= 1)
			...
			offscreen.fill(new Ellipse2D.Double(scaleX(x) - scaledPenRadius / 2, scaleY(y) - scaledPenRadius / 2,
	...
	private static void polygon(double[] x, double[] y, boolean close, boolean fill) {
		...
		if (y.length != N)
			throw new IllegalArgumentException("x[] and y[] must have the same number of elements.  " + "x[] has "
					+ x.length + " elements, but y[] has " + y.length + " elements.");

		if ((close || fill) && N < 3)
			throw new IllegalArgumentException(
					"You must specify at least three for a polygon.  " + "You have only provided " + N + " points.");
	...
	/**
	 * Draw picture (gif, jpg, or png) centered on (x, y), rotated given number of
	 * degrees, rescaled to w-by-h. If w and h are both 0, use the original image
	 * dimensions. If one is 0, scale the image to match the other dimension.
	 * 
	 * @param x       the center x-coordinate of the image
	 * @param y       the center y-coordinate of the image
	 * @param s       the name of the image/picture, e.g., "ball.gif"
	...
	/**
	 * Set animation mode with specified frame rate Equivalent to calling
	 * PennDraw.show(0), with subsequent calls to PennDraw.advance() being
	 * equivalent to calling PennDraw.show(10000.0 / frameRate).
	 *
	 * Use PennDraw.enableAnimation(0) to have PennDraw.advance() draw as fast as
	 * possible (just like PennDraw.show(0)).
	 * 
	...
	/**
	 * Indicate that all drawing commands for the next frame are complete so it can
	 * be drawn
	 *
...
//		PennDraw.filledSquare(0.1,0.1,0.1);
		PennDraw.setPenColor(226, 140, 59);
		PennDraw.filledSquare(0.125, 0.875, 0.125);
		PennDraw.filledSquare(0.375, 0.875, 0.125);
		PennDraw.filledSquare(0.625, 0.875, 0.125);
		PennDraw.filledSquare(0.875, 0.875, 0.125);
